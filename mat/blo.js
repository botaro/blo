class Block extends BlockCore {
  ...
  draw() {
    if (this.body) {
      this.update();
      if (this.attributes.color || this.attributes.stroke) super.draw();
      if (this.attributes.image) this.drawSprite();
...
if (c.draw === true) this.drawConstraint(c);
  ...
  drawSprite() {
    ...
    rotate(angle);
    imageMode(CENTER);
image(this.attributes.image, this.offset.x, this.offset.y, this.attributes.image.width *1/2* this.attributes.scale, this.attributes.image.height*1/2 * this.attributes.scale);
pop();
}
